...
- [ACTION:OPEN_APP] app_name — when user wants to open an installed application on their laptop (e.g. notepad, calculator, spotify, code, etc.)
- [ACTION:WRITE_NOTEPAD] text — when user wants to write text/notes into Notepad on Windows (e.g. "write hello world in notepad", "type my shopping list in notepad")
...
def _fetch_weather_string_sync() -> Optional[str]:
    ...
    if api_key:
        try:
            owm_unit = "imperial" if unit == "fahrenheit" else "metric"
            url = (
                "https://api.openweathermap.org/data/2.5/weather"
                f"?lat={location['latitude']}&lon={location['longitude']}"
                f"&appid={api_key}&units={owm_unit}"
            )
            import urllib.request as _ureq
            ...
            if temp is not None:
                ...
                if data.get("weather") and len(data["weather"]) > 0:
                    weather_desc = data["weather"][0].get("description", "").strip()
                desc_str = f" ({weather_desc})" if weather_desc else ""
                return f"Current weather in {location['label']}: {temp}{unit_symbol}{desc_str}"
        except Exception as e:
            log.warning(f"OpenWeatherMap fetch failed, falling back to Open-Meteo: {e}")
    ...
    try:
        response = await client.messages.create(
            ...
            system=(
                ...
                "- Open applications installed on the computer (like Notepad, Spotify, Calculator, Slack, Zoom, VS Code, Paint, Word, etc.)\n"
                "- Write text/notes into Notepad on Windows (e.g. \"write hello world in notepad\", \"type my shopping list in notepad\")\n\n"
                "Note: speech-to-text may produce errors like \"Cloud\" for \"Claude\", "
                "\"Travis\" for \"JARVIS\", \"clock code\" for \"Claude Code\".\n\n"
                "Return ONLY valid JSON: {\"action\": \"open_terminal|browse|build|open_app|write_notepad|chat\", "
                "\"target\": \"the text content to write or details of application to open\"}\n"
                ...
                "open_app = user wants to open an installed application (e.g. Notepad, Calculator, Spotify, VS Code/Code, Paint, Slack, Zoom)\n"
                "write_notepad = user wants to write text/notes into Notepad (the target should contain the clean text content to write)\n"
                "chat = conversation, greetings, or questions about current weather, time, or general facts\n"
...
def extract_action(response: str) -> tuple[str, dict | None]:
    ...
    match = _action_re.search(
        r'\[ACTION:(BUILD|BROWSE|RESEARCH|OPEN_TERMINAL|OPEN_APP|WRITE_NOTEPAD|PROMPT_PROJECT|ADD_TASK|ADD_NOTE|COMPLETE_TASK|REMEMBER|CREATE_NOTE|READ_NOTE|SCREEN)\]\s*(.*?)$',
...
async def _execute_browse(target: str):
    """Execute a browse action — smartly routes to YouTube, Google search, or direct URL.

    Routing rules (in priority order):
      1. Explicit full URL  → open directly
      2. Domain-like string → prepend https://
      3. YouTube search    → youtube.com/search
      4. Known site name   → https://www.{site}.com
      5. Everything else   → Google search
    """
    try:
        ...
        if t.startswith("http://") or t.startswith("https://"):
            await open_browser(t)
            return

        # 2. Looks like a bare domain (github.com, reddit.com/r/python, …)
        first_word = t.split()[0] if t.split() else t
        if "." in first_word and not first_word.startswith("."):
            await open_browser(f"https://{t}")
            return

        # 3. YouTube search — "search youtube for X" / "play X on youtube"
        yt_patterns = [
        ...
        for pat in yt_patterns:
            m = _bre.search(pat, t_low)
            if m:
                query = m.group(1).strip()
                await open_browser(f"https://www.youtube.com/results?search_query={quote(query)}")
                return

        # 4. Known bare site name → homepage
        known_sites = {
            "youtube", "github", "google", "facebook", "twitter", "x",
            "wikipedia", "reddit", "linkedin", "netflix", "amazon", "yahoo",
            "bing", "instagram", "chatgpt", "stackoverflow", "gmail",
            "outlook", "twitch", "discord", "spotify", "whatsapp",
        }
        if t_low in known_sites:
            if t_low == "x":
                await open_browser("https://www.x.com")
            elif t_low == "chatgpt":
                await open_browser("https://chat.openai.com")
            else:
                await open_browser(f"https://www.{t_low}.com")
            return

        # 5. Fallback → Google search
        await open_browser(f"https://www.google.com/search?q={quote(t)}")
...
async def _execute_open_app(target: str):
    ...
    try:
        await handle_open_app(target)
    except Exception as e:
        log.error(f"Open app failed: {e}")


async def _execute_write_notepad(target: str):
    """Execute a write-notepad action from an LLM-embedded [ACTION:WRITE_NOTEPAD] tag."""
    try:
        await handle_write_notepad(target)
    except Exception as e:
        log.error(f"Write notepad failed: {e}")
...
def _get_kokoro():
    ...
    if not onnx_path.exists() or not bin_path.exists():
        log.warning("Kokoro model files kokoro-v1.0.onnx or voices-v1.0.bin are missing. Run download_kokoro.py first.")
        return None
        
    try:
        from kokoro_onnx import Kokoro
        log.info("Initializing Kokoro TTS model...")
        _kokoro_instance = Kokoro(str(onnx_path), str(bin_path))
...
async def synthesize_speech(text: str) -> Optional[bytes]:
    ...
    kokoro = _get_kokoro()
    if not kokoro:
        log.warning("Kokoro TTS not ready, skipping synthesis")
        return None

    try:
        def _infer():
            # Generate raw audio samples (bm_george is a British male voice)
            ...
            # Convert float32 samples to standard WAV bytes in memory
            import io
            import wave
            import numpy as np
        ...
        # Run in thread pool executor to avoid blocking FastAPI async loop
        loop = asyncio.get_running_loop()
        wav_bytes = await loop.run_in_executor(None, _infer)
        ...
        log.error(f"Kokoro TTS error: {e}", exc_info=True)
        return None
...
# ---------------------------------------------------------------------------
# Groq API Client Bridge (replaces Anthropic client transparently)
# ---------------------------------------------------------------------------

class GroqMessagesBridge:
    def __init__(self, api_key: str):
        self.api_key = api_key

    async def create(self, model: str, max_tokens: int, messages: list, system: str = None, **kwargs):
        # Format the system prompt into OpenAI style if present
        formatted_messages = []
        if system:
            formatted_messages.append({"role": "system", "content": system})
        
        # Detect if any message has image content (vision call)
        has_image = False
        for msg in messages:
            ...
            if isinstance(content, list):
                ...
                for item in content:
                    if isinstance(item, dict):
                        ...
                        new_content.append(item)
                formatted_messages.append({"role": msg.get("role"), "content": new_content})
        ...
        # Robust JSON cleaning/extraction if JSON is expected
        is_json_expected = False
        if system and ("json" in system.lower() or "array" in system.lower() or "object" in system.lower()):
            is_json_expected = True
        for m in formatted_messages:
            content_str = m.get("content")
            if isinstance(content_str, str) and ("json" in content_str.lower() or "array" in content_str.lower()):
                is_json_expected = True
        ...
        # Mock classes to match Anthropic API response structure
        ...
        class MockResponse:
            def __init__(self, text_val, in_tokens, out_tokens):
                self.content = [MockContentBlock(text_val)]
                self.usage = MockUsage(in_tokens, out_tokens)

        return MockResponse(text, input_tokens, output_tokens)


class GroqClientBridge:
    def __init__(self, api_key: str):
        self.messages = GroqMessagesBridge(api_key)
...
'''],
                        ...
                        try:
                            from screen import _get_windows_win32
                            windows = _get_windows_win32()
                            if windows:
                                _ctx_cache["screen"] = format_windows_for_context(windows)
                        except Exception:
                            pass
...
async def lifespan(application: FastAPI):
    ...
    if GROQ_API_KEY:
        anthropic_client = GroqClientBridge(api_key=GROQ_API_KEY)
    else:
...
def detect_action_fast(text: str) -> dict | None:
    ...
    # ── Browser / Web ──────────────────────────────────────────────────────
    import re as _daf_re

    # YouTube search — "search youtube for lofi music"
    _yt = _daf_re.search(
        r"(?:search\s+youtube\s+for|youtube\s+search\s+for|search\s+for\s+(.+)\s+on\s+youtube"
        r"|play\s+(.+)\s+on\s+youtube|find\s+(.+)\s+on\s+youtube)\s*(.*)",
        t,
    )
    if _yt:
        # extract the query from whichever group matched
        query = (_yt.group(1) or _yt.group(2) or _yt.group(3) or _yt.group(4) or "").strip()
        if query:
            return {"action": "browse", "target": f"search youtube for {query}"}

    # Google search — "search for black holes" / "google black holes"
    _gs = _daf_re.match(r"(?:search\s+for|google|search)\s+(.+)", t)
    if _gs:
        query = _gs.group(1).strip()
        if query and "youtube" not in query:          # don't double-handle YouTube
            from urllib.parse import quote as _q
            return {"action": "browse", "target": f"https://www.google.com/search?q={_q(query)}"}

    # Open website by domain — "open github.com" / "open reddit.com/r/python"
    _dom = _daf_re.match(r"open\s+(\S+\.\S+.*)", t)
    if _dom:
        site = _dom.group(1).strip()
        return {"action": "browse", "target": site}

    # Open known site by name — "open youtube" / "open github"
    _known_sites = {
        "youtube", "github", "google", "facebook", "twitter", "x",
        "wikipedia", "reddit", "linkedin", "netflix", "amazon", "yahoo",
        "bing", "instagram", "chatgpt", "stackoverflow", "gmail",
    ...
    _open_m = _daf_re.match(r"open\s+(\w+)$", t)
    if _open_m and _open_m.group(1) in _known_sites:
        return {"action": "browse", "target": _open_m.group(1)}
    # ─────────────────────────────────────────────────────────────────────

    # Open application
    if any(p in t for p in ["open notepad", "open calculator", "open calc", "open spotify", "open paint", "open slack", "open zoom", "open vs code", "open vscode", "open edge", "open firefox"]):
        parts = t.split("open ", 1)
        if len(parts) == 2:
            return {"action": "open_app", "target": parts[1].strip()}

    # Write to Notepad
    if "write" in t and "notepad" in t:
        import re
        match = re.search(r'write\s+(.*?)\s+in\s+notepad', t, re.IGNORECASE)
        if not match:
            match = re.search(r'write\s+(?:to|in)\s+notepad\s+(.*)', t, re.IGNORECASE)
        if not match:
            match = re.search(r'open\s+notepad\s+and\s+write\s+(.*)', t, re.IGNORECASE)
        if match:
            return {"action": "write_notepad", "target": match.group(1).strip()}
    ...
    # ── Volume control ──────────────────────────────────────────────────
    if any(p in t for p in ["volume up", "turn up the volume", "increase volume", "louder"]):
        return {"action": "volume", "target": "up"}
    if any(p in t for p in ["volume down", "turn down the volume", "decrease volume", "quieter", "lower the volume"]):
        return {"action": "volume", "target": "down"}
    if any(p in t for p in ["mute", "mute the volume", "silence"]):
        return {"action": "volume", "target": "mute"}
    if any(p in t for p in ["unmute", "unmute volume"]):
        return {"action": "volume", "target": "unmute"}

    # ── Power / system state ──────────────────────────────────────────────
    if any(p in t for p in ["shutdown", "shut down", "shut the computer down", "power off", "turn off the computer"]):
        return {"action": "power", "target": "shutdown"}
    if any(p in t for p in ["restart", "reboot", "restart the computer", "reboot the computer"]):
        return {"action": "power", "target": "restart"}
    if any(p in t for p in ["sleep", "put to sleep", "sleep mode", "hibernate"]):
        return {"action": "power", "target": "sleep"}

    # ── Remember this / save note ──────────────────────────────────────────
    import re as _mem_re
    _remember_m = _mem_re.match(
        r"(?:remember|note|save|keep a note|make a note)[:\s]+(?:that\s+)?(.+)", t
    )
    if _remember_m:
        return {"action": "remember", "target": _remember_m.group(1).strip()}
    if t in ("remember this", "save this", "note this", "keep this"):
        return {"action": "remember", "target": "__last__"}

    # ── Clipboard read aloud ────────────────────────────────────────────────
    if any(p in t for p in ["read clipboard", "read my clipboard", "what's in my clipboard",
                             "whats in my clipboard", "read what i copied", "what did i copy",
                             "read this", "what's copied", "whats copied"]):
    ...
    _browser_cmds = {
        ...
        # tabs
        "next tab", "previous tab", "prev tab", "new tab", "close tab", "open new tab",
        # navigation
        "go back", "go forward",
        # refresh
        "refresh", "reload", "refresh page", "reload page",
        # zoom
        "zoom in", "zoom out", "zoom reset", "reset zoom",
        # other
        "incognito", "open incognito", "private window",
        "address bar", "focus address bar", "search bar",
    }
    if t in _browser_cmds:
        return {"action": "browser_control", "target": t}
    # Also catch "scroll down a bit", "go back please" etc.
    for _bc in _browser_cmds:
        if t.startswith(_bc):
            return {"action": "browser_control", "target": _bc}
...
async def handle_volume(direction: str) -> str:
    """Control system volume using Windows virtual key codes via PowerShell."""
    import sys
    if sys.platform != "win32":
        return "Volume control is only supported on Windows, sir."
    # VK codes: 174=vol_down, 175=vol_up, 173=mute
    vk_map = {"up": 175, "down": 174, "mute": 173, "unmute": 173}
    vk = vk_map.get(direction, 175)
    # Send the key press 3 times for up/down, once for mute
    presses = 3 if direction in ("up", "down") else 1
    try:
        import subprocess
        ps_code = (
            f"$wsh = New-Object -ComObject WScript.Shell; "
            + "; ".join([f'$wsh.SendKeys([char]{vk})'] * presses)
        )
        subprocess.Popen(["powershell", "-Command", ps_code],
                         stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
        labels = {"up": "turned up", "down": "turned down", "mute": "muted", "unmute": "unmuted"}
        return f"Volume {labels.get(direction, 'adjusted')}, sir."
    except Exception as e:
        log.error(f"Volume control failed: {e}")
        return "I had trouble adjusting the volume, sir."


async def handle_power(action: str) -> str:
    ...
    try:
        if sys.platform == "win32":
            ...
            cmd = cmds.get(action)
            if not cmd:
                return "Unknown power action, sir."
            subprocess.Popen(cmd)
            msgs = {
                "shutdown": "Shutting down in 5 seconds, sir. Goodnight.",
                "restart":  "Restarting in 5 seconds, sir.",
                "sleep":    "Putting the system to sleep, sir.",
            }
            return msgs.get(action, "Done, sir.")
        else:
            return "Power control is only supported on Windows, sir."
    except Exception as e:
        log.error(f"Power action failed: {e}")
        return "I had trouble executing that power command, sir."


async def handle_remember(content: str, last_response: str = "") -> str:
    """Save a note/fact to JARVIS memory (SQLite)."""
    from memory import remember, create_note
    try:
        text = last_response.strip() if content == "__last__" else content
        if not text:
            return "There's nothing to remember, sir."
        remember(text, mem_type="fact", source="voice_command", importance=7)
        short = text[:60] + "..." if len(text) > 60 else text
        return f"Noted and stored, sir. I'll remember: {short}"
    except Exception as e:
        log.error(f"Remember failed: {e}")
        return "I had trouble saving that, sir."


async def handle_read_clipboard() -> str:
    ...
    try:
        ...
        # Trim very long clipboard content for voice
        if len(text) > 400:
            text = text[:400] + "... and more."
        return f"Clipboard contains: {text}"
    except Exception as e:
        log.error(f"Clipboard read failed: {e}")
        return "I couldn't read the clipboard, sir."


async def handle_browser_control(command: str) -> str:
    """Delegate a browser tab control command to browser_control module."""
    try:
        from browser_control import execute
        return await execute(command)
    except ImportError:
        return "Browser control module not found, sir."
    except Exception as e:
...
async def handle_write_notepad(target: str) -> str:
    from actions import write_notepad
                ...
                # ── CHAT MODE: fast keyword detection + Haiku ──
                    ...
                    if action:
                        if action["action"] == "open_terminal":
                            ...
                            # Build a natural spoken confirmation
                            t_low = target.lower()
                            if "youtube.com/results" in target or "search youtube for" in t_low:
                                query = t_low.replace("search youtube for", "").strip()
                                response_text = f"Searching YouTube for {query}, sir."
                            elif "google.com/search" in target:
                                import re as _cr
                                m = _cr.search(r"[?&]q=([^&]+)", target)
                                query = m.group(1).replace("+", " ") if m else target
                                response_text = f"Searching Google for {query}, sir."
                            else:
                            ...
                            response_text = await handle_volume(action["target"])
                        elif action["action"] == "power":
                            response_text = await handle_power(action["target"])
                        elif action["action"] == "remember":
                            response_text = await handle_remember(action["target"], last_response=last_jarvis_response)
                        elif action["action"] == "read_clipboard":
                            response_text = await handle_read_clipboard()
                        elif action["action"] == "browser_control":
                            response_text = await handle_browser_control(action["target"])
                        else:
                        ...
                        if not anthropic_client:
                            ...
                            if embedded_action:
                                ...
                                if not response_text.strip():
                                    ...
                                    elif action_type == "write_notepad":
                                        response_text = "Writing that in Notepad, sir."
                                    else:
...
async def api_settings_keys(body: KeyUpdate):
    allowed = {"GROQ_API_KEY", "FISH_API_KEY", "FISH_VOICE_ID", "USER_NAME", "HONORIFIC", "CALENDAR_ACCOUNTS", "OPENWEATHERMAP_API_KEY"}
...
async def api_test_groq(body: KeyTest):
    ...
    try:
        client = GroqClientBridge(api_key=key)
        await client.messages.create(model="llama-3.1-8b-instant", max_tokens=10, messages=[{"role": "user", "content": "Hi"}])
        return {"valid": True}
    except Exception as e:
...
@app.post("/api/settings/test-openweathermap")
async def api_test_openweathermap(body: KeyTest):
    key = body.key_value or os.getenv("OPENWEATHERMAP_API_KEY", "")
    if not key:
        return {"valid": False, "error": "No key provided"}
    try:
        import urllib.request as _ureq
        ...
        try:
            with _ureq.urlopen(url, timeout=5.0) as resp:
                if resp.status == 200:
                    return {"valid": True}
        except _uerr.HTTPError as he:
            if he.code == 401:
                return {"valid": False, "error": "Invalid API key"}
            return {"valid": False, "error": f"HTTP {he.code}: {he.reason}"}
        return {"valid": True}
    except Exception as e:
        return {"valid": False, "error": str(e)[:200]}
...
async def api_settings_status():
    ...
    return {
        ...
        "env_keys_set": {
            "groq": bool(env_dict.get("GROQ_API_KEY", "").strip() and env_dict.get("GROQ_API_KEY", "") != "your-groq-api-key-here"),
            "fish_audio": _get_kokoro() is not None,
            "fish_voice_id": True,
