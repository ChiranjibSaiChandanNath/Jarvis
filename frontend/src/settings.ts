    ...
    <div class="settings-panel" id="settings-panel-inner">
      ...
      <div class="settings-body">
        ...
        <section class="settings-section" id="section-api-keys">
          ...
          <!-- Kokoro TTS (Local) status -->
          <div class="settings-field">
            <label>Kokoro TTS (Local British Accent)</label>
            <div class="settings-input-row" style="align-items: center; gap: 8px; display: flex;">
              <span class="status-dot" id="status-fish"></span>
              <span id="label-kokoro-status" style="font-size: 0.95em; color: #888; margin-left: 8px;">Checking model status...</span>
            </div>
          </div>

          <!-- Hidden legacy Fish Audio fields to keep JS event listeners happy -->
          <div style="display: none;">
            <input type="password" id="input-fish-key" value="local-kokoro" />
            <button id="btn-test-fish"></button>
            <input type="text" id="input-fish-voice-id" value="bm_george" />
            <button id="btn-save-voice-id"></button>
          </div>
...
async function loadStatus() {
  try {
    ...
    const isKokoroReady = status.env_keys_set.fish_audio;
    setDotStatus("status-fish", isKokoroReady ? "green" : "red");
    const kokoroLabel = document.getElementById("label-kokoro-status");
    if (kokoroLabel) {
      kokoroLabel.textContent = isKokoroReady ? "Local Model Active" : "Models downloading or missing";
      kokoroLabel.style.color = isKokoroReady ? "#00ffcc" : "#ff3366";
    }
